...
rsaApi.post('/secret/upload', async(ctx, next) => {
    try {
        const msg = ctx.request.body
        if (!msg.des) {
            throw '没有des密码'
        ...
        const file = ctx.request.files.file
        let res = fs.readFileSync(file.path, 'utf-8')
        let utf8= CryptoJS.enc.Utf8.parse(String(res));
        var base64String = CryptoJS.enc.Base64.stringify(utf8);
        var hash = CryptoJS.MD5(base64String).toString().substring(8, 24);
